...
(function($) {
    ...
    $(document).on('click', '.navbar-nav .ripple-group', function(event) {
        if (location.pathname != '/') {
            event.preventDefault();
            event.stopPropagation();
            if ($(this).attr('href') == $('#base-url').val()+"blog")
                window.location = $(this).attr('href');
            else
                window.location = $('#base-url').val()+$(this).attr('href');
        }
    }); 
    ...
    function init_map() {
        if ($("#map-section").length) {
            ...
            var mapZoom = 14, // mapLatitude = 40.679418,
                ...
                firstContentString = "<div class='gmap-popup'>" + "<h4>" + "Heraclea" + "</h4>" + "<p>" + "Cra 13 # 71-57 <br> Bogotá Colombia" + "</p>" + "<p>" + "" + "</p>"+ "</div>";
            ...
            var markerIcon = (is_internetExplorer11) ? 'assets/img/google-map-assets/map-icon-location.png' : 'assets/img/google-map-assets/map-icon-location.svg';
